...
MODEL_CLASSES = ["pix2pix", "cyclegan", "wav2lip", "esrgan", "edvr", "fom", "stylegan2", "basicvsr","rcan"]
...
def main():
    ...
    for i in range(max_eval_steps):
        ...
        if model_type == "pix2pix":
            ...
            save_image(image_numpy, os.path.join(args.output_path, "rcan/{}.png".format(i)))

            gt_img = tensor2img(gt_tensor, min_max)
            out_img = image_numpy

            metric_file = os.path.join(args.output_path, "rcan/metric.txt")
            for metric in metrics.values():
